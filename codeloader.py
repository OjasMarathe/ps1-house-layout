...
_SBC_FIELDS = {f.name for f in fields(SBCConstraints)}


def sbc_from_dict(ext: dict) -> SBCConstraints:
    """Build an SBCConstraints from a raw constraint dict (e.g. the Constraint
    Engine's `exterior` block). UNKNOWN KEYS ARE IGNORED, so the Constraint
    Engine can add new constraint types without breaking the verifier."""
    # skip unknown keys AND null values (a ruleset may send `null` = "no limit")
    known = {k: float(v) for k, v in ext.items() if k in _SBC_FIELDS and v is not None}
    return SBCConstraints(**known)
